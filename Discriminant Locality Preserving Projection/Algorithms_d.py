...
###############################LPP算法函数######################################
#####################计算自适应epsilon graph####################
def compute_adaptive_neighbors(Data, threshold):
...
def adaptive_epsilon_graph(Data, method, k ,threshold):
    adaptive_neighbors, sorted_distances, distances = compute_adaptive_neighbors(Data, threshold)
    n = Data.shape[1]  
...
def compute_neighborhood_matrix(Data, method, k, threshold):
    n = Data.shape[1]  # 获取样本点的数量
    if method == "KNN":
        knn_adjacency_matrix, distances = knn_graph(Data, method, k)
        return knn_adjacency_matrix, distances
    elif method == "Adaptive epsilon":
        adaptive_epsilon_adjacency_matrix, distances = adaptive_epsilon_graph(Data, method, k ,threshold)
        return adaptive_epsilon_adjacency_matrix, distances
        ...
        epsilon_adjacency_matrix, distances = adaptive_epsilon_graph(Data, method, k ,threshold)
        return epsilon_adjacency_matrix, distances

def construct_weight_matrix(Data, method, k,t, threshold):
    n = Data.shape[1]  
    Weight_matrix = np.zeros((n, n))
    adjacency_matrix, distances = compute_neighborhood_matrix(Data, method, k, threshold)
...
def LPP(Data, d, method, k, t, threshold):
    Data = Data.T
    Weight_matrix = construct_weight_matrix(Data, method, k,t, threshold)
